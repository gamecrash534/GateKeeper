...
public class AddCommand {
    ...
    private static int execute(CommandContext<CommandSource> ctx) {
        ...
        Player player = server.getPlayer(argumentPlayer).isPresent() ? server.getPlayer(argumentPlayer).get() : null;

        if (player == null) {
            ctx.getSource().sendRichMessage("<dark_gray>[<yellow>Whitelist<dark_gray>] <red>The specified player is not online.");
        }
